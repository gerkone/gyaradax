...
def main():
    ...
    def run_lto_v3(d, p):
        p_lto = (pre["bessel"] * p.reshape(1, 1, -1, nkx, nky)).reshape(-1, nkx, nky)
        out = lto_bracket_ffi_call(d, p_lto, kx_vec, ky_vec, inverse_jind, dum_s, batch_total, mrad, mphi, nkx, nky, 3)
    ...
    def run_lto_vz2z(d, p):
        ...
        )(d, p_lto, kx_vec, ky_vec, inverse_jind, dum_s,
    ...
    for name, fn, inputs in variants:
        try:
            ...
            if ref_out is not None:
                N_acc = 1000
                rel_err = jnp.linalg.norm((out_flat.ravel() - ref_flat.ravel())[:N_acc]) / jnp.linalg.norm(ref_flat.ravel()[:N_acc])
