...
// ── Interleaved Z2Z Load Callback (for single-FFT optimization) ─────────────
// Packs df and phi into INTERLEAVED batches:
//   batch 2*b   = df[b] derivatives (fy + i*fx)
//   batch 2*b+1 = phi[b] derivatives (fy + i*fx)
//
// This allows the store callback to read both df and phi from adjacent batches.

__device__ cufftComplex d_v5_z2z_interleaved_fp32_load(
    void *dataIn, unsigned long long offset,
    void *callerInfo, void *sharedPointer)
{
    const V5Z2zInfo* ci = (const V5Z2zInfo*)callerInfo;
    ...
    // Determine if this is df (even) or phi (odd) batch
    int is_phi = gb % 2;
    int lb     = gb / 2;  // Local batch index within df or phi
    
    // Check bounds
    if (is_phi && lb >= ci->b_phi) return make_float2(0.0f, 0.0f);
    if (!is_phi && lb >= ci->b_df) return make_float2(0.0f, 0.0f);
    
    const double2* field = is_phi ? ci->phi_packed : ci->df_packed;
    int field_batch_size = is_phi ? ci->b_phi : ci->b_df;
    if (field_batch_size == 0) return make_float2(0.0f, 0.0f);
    
    // Cycle through phi batches if b_phi < b_df
    int actual_lb = lb % field_batch_size;
    ...
    double2 val = v5_get(field, actual_lb, kxi, j_src, ci->nkx, ci->nky);
    double kxv = ci->kx[kxi];
    double kyv = ci->ky[j_src];
    ...
    if ((j_src == 0 || j_src == ci->mphi / 2) && !mirror) {
        ...
        int kx_pair = ci->inverse_jind[m_pair];
        if (kx_pair >= 0 && m_pair != i) {
            double2 vp = v5_get(field, actual_lb, kx_pair, j_src, ci->nkx, ci->nky);
            val.x = 0.5 * (val.x + vp.x);
            val.y = 0.5 * (val.y - vp.y);
        }
    }
    ...
    // Pack: fy + i*fx
    if (!mirror)
        return make_float2((float)(fy_re - fx_im), (float)(fy_im + fx_re));
    else
        return make_float2((float)(fy_re + fx_im), (float)(fx_re - fy_im));
}

__device__ cufftJITCallbackLoadC d_v5_z2z_interleaved_fp32_load_addr = d_v5_z2z_interleaved_fp32_load;

// ── True FP32 Interleaved Z2Z Load Callback (Early Cast Optimization) ───────
// Same optimization as d_v5_z2z_true_fp32_load but for interleaved batches:
//   batch 2*b   = df[b] derivatives
//   batch 2*b+1 = phi[b] derivatives

__device__ cufftComplex d_v5_z2z_interleaved_true_fp32_load(
    void *dataIn, unsigned long long offset,
    void *callerInfo, void *sharedPointer)
{
    const V5Z2zInfo* ci = (const V5Z2zInfo*)callerInfo;
    ...
    // CAST IMMEDIATELY: double2 -> float2
    double2 val_d = v5_get(field, actual_lb, kxi, j_src, ci->nkx, ci->nky);
    float2 val = make_float2((float)val_d.x, (float)val_d.y);
    ...
    if ((j_src == 0 || j_src == ci->mphi / 2) && !mirror) {
        ...
        int kx_pair = ci->inverse_jind[m_pair];
        if (kx_pair >= 0 && m_pair != i) {
            double2 vp_d = v5_get(field, actual_lb, kx_pair, j_src, ci->nkx, ci->nky);
            float2 vp = make_float2((float)vp_d.x, (float)vp_d.y);
            val.x = 0.5f * (val.x + vp.x);
            val.y = 0.5f * (val.y - vp.y);
        }
...
__device__ cufftJITCallbackLoadC d_v5_z2z_interleaved_true_fp32_load_addr = d_v5_z2z_interleaved_true_fp32_load;
