...
@wp.kernel
def scale_kernel(a: wp.array(dtype=wp.float64),
...
def run_micro():
    ...
    return {
        "single_jax": t_jax, "single_warp": t_wp,
        "scan_jax": t_jax_scan, "scan_warp": t_wp_scan,
...
def run_production(config, mixed_precision):
    ...
    def warp_stencil_cb(inputs, outputs, attrs, ctx):
        ...
        wp_out = wp.array(ptr=out_ptr, dtype=wp.vec2d,   shape=(nv_nmu * ns * nkx * nky,), device="cuda")
        stream = wp.Stream(device="cuda:0", cuda_stream=ctx.stream)
        with wp.ScopedStream(stream):
    ...
    print(f"\n  lax.scan ({N_SCAN} iters, per-iteration, 30 trials):")
    t_jax_scan = _bench_scan(lambda: jax_scan(field, coeffs_broadcasted, N_SCAN).block_until_ready(), n_iters=N_SCAN)
    t_wp_scan  = _bench_scan(lambda: warp_scan(field, coeffs_raw, N_SCAN).block_until_ready(), n_iters=N_SCAN)
    scan_overhead = t_wp_scan['mean'] - t_jax_scan['mean']
    print(f"    {'JAX':25s}  {_stats_str(t_jax_scan)}")
    print(f"    {'Warp ffi_callback':25s}  {_stats_str(t_wp_scan)}   {scan_overhead:+.1f} μs/iter")
...
def run(config="configs/iteration_13.yaml", mixed_precision=False):
    ...
    print(f"  │   JAX                   │ {micro['single_jax']['mean']:>8.0f} μs         │ {micro['scan_jax']['mean']:>8.1f} μs/iter  │")
    print(f"  │   Warp FFI              │ {micro['single_warp']['mean']:>8.0f} μs         │ {micro['scan_warp']['mean']:>8.1f} μs/iter  │")
    print(f"  │   → overhead            │ {micro['overhead_single']:>+8.0f} μs         │ {micro['overhead_scan']:>+8.1f} μs/iter  │")
    print(f"  ├─────────────────────────┼────────────────────┼────────────────────┤")
    print(f"  │ PRODUCTION (stencil)    │                    │                    │")
    print(f"  │   JAX                   │ {prod['single_jax']['mean']:>8.0f} μs         │ {prod['scan_jax']['mean']:>8.1f} μs/iter  │")
    print(f"  │   Warp FFI              │ {prod['single_warp']['mean']:>8.0f} μs         │ {prod['scan_warp']['mean']:>8.1f} μs/iter  │")
    print(f"  │   → overhead            │ {prod['overhead_single']:>+8.0f} μs         │ {prod['overhead_scan']:>+8.1f} μs/iter  │")
    print(f"  └─────────────────────────┴────────────────────┴────────────────────┘")

    # Interpret results
    print(f"\n  Interpretation:")
    if micro['overhead_single'] > 100:
        print(f"    • Micro single-call shows +{micro['overhead_single']:.0f}μs Python callback overhead")
    else:
        print(f"    • Micro single-call shows {micro['overhead_single']:.0f}μs — callback overhead is minimal")

    if micro['overhead_scan'] < micro['overhead_single'] * 0.3:
        print(f"    • CUDA graph capture ELIMINATES overhead: "
              f"{micro['overhead_single']:.0f}μs → {micro['overhead_scan']:.1f}μs in scan")
        print(f"      ({micro['overhead_single']/(max(abs(micro['overhead_scan']),0.1)):.0f}× reduction)")
    elif micro['overhead_scan'] < micro['overhead_single'] * 0.7:
        print(f"    • CUDA graph capture REDUCES overhead: "
              f"{micro['overhead_single']:.0f}μs → {micro['overhead_scan']:.1f}μs in scan")
    else:
        print(f"    • CUDA graph capture did NOT reduce overhead: "
              f"{micro['overhead_single']:.0f}μs → {micro['overhead_scan']:.1f}μs in scan")
