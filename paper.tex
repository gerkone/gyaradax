...
%
Finally, in the last sections we show numerical validation of the solver on empirical trajectories generated with GKW \footnote{\textbf{\url{https://bitbucket.org/gkw/gkw}}} (\cref{sec:verification}),
and some differentiable programming experiments (\cref{sec:exp}).
...
%
The final performance optimizations was performed by the authors, with advice and quick experimentation with Claude Code. Some opportunities for speedup, such as the mixed precision FFT, were spotted and implemented entirely by Claude.
...
\begin{figure*}[t]
    ...
    \caption{Empirical validation of \texttt{gyaradax} against GKW for two adiabatic ITG equilibria (left: $q{=}1.41$, $\hat{s}{=}0.80$; right: $q{=}7.73$, $\hat{s}{=}2.14$).
    \textbf{Top:} heat and momentum flux traces. Both codes coincide during linear growth; after nonlinear saturation the trajectories diverge due to chaotic sensitivity but converge to the same time-averaged flux levels.
    \textbf{Bottom:} time-averaged $k_y$ and $k_x$ spectral profiles, showing agreement across four decades in amplitude including peak location, inertial range slope, and dissipation-scale rolloff.
    }
    \label{fig:empirical}
\end{figure*}
...
\textbf{Wavenumber spectra.}
The spectral energy distribution across perpendicular wavenumbers characterizes the turbulent cascade.
We compare the time-averaged spectral profiles $k_y^{\text{spec}}(k_y) = \sum_{s, k_x} |\hat{\phi}(s, k_x, k_y)|^2$ and $k_x^{\text{spec}}(k_x) = \sum_{s, k_y} |\hat{\phi}(s, k_x, k_y)|^2$, which measure how energy is distributed across binormal and radial scales respectively.
\Cref{fig:emp_spectras} (bottom) shows that \texttt{gyaradax} reproduces the GKW spectras.
...
\begin{figure}[h]
    ...
    \caption{Per-$k_y$ growth rate profiles $\gamma(k_y)$ at three simulation snapshots and time-averaged (bottom right). The amplitude is computed over the connected $k_x$ chain containing $k_x{=}0$, matching the GKW convention.}
...
\begin{figure}[t]
    ...
    \caption{Kinetic electron validation for the low-drive case ($R/L_{T,e} = 3.45$).
    \textbf{Top:} per-species transport fluxes (ion, left; electron, right).
    \textbf{Bottom:} time-averaged $k_y$ and $k_x$ spectral profiles.
    \texttt{gyaradax} (color) vs GKW reference (dashed).}
...
\label{sec:exp}
As a demonstration of the differentiable, ML-oriented nature of this solver, we show some examples of differentiable programming.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\columnwidth]{figures/sensitivity_gamma.pdf}\\
    ...
    \textbf{Bottom:} AD-computed sensitivity $\partial\gamma / \partial(R/L_T)$.}
    \label{fig:sensitivity}
\end{figure}

\textbf{Neural correction.}
\label{sec:correction}
As a proof-of-concept for solver + ML hybrid, we train a small neural network to correct the transport of a spectrally truncated simulation, similarly to what Solver in the Loop \cite{um2021solverinthelooplearningdifferentiablephysics} did in the temporal axis, to achieve long term stability with large timesteps.
%
...
After each solver block, an MLP applies a $k_y$ mode-wise multiplicative correction, with the heat flux MSE as target.
