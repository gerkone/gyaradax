...
\twocolumn[
\icmltitle{
\raisebox{-2.0pt}{\includegraphics[width=1.1em]{figures/gyarados_logo.png}}\hspace{0.2em}%
...
We developed \texttt{gyaradax} with heavy usage of coding agents \cite{??} and \textit{vibecoding}. This comes from the recent shift in software engineering, and our success with this code shows scientific software is no different.
%
By coupling (aspiring) expert humans with coding agents, we demonstrate that complex reimplementations of numerical kernels can be bootstrapped in a fraction of the time, freeing time and resources for more fundamental research. We consider worth mentioning, that the entirety of this project was developed within \textit{two weeeks}, and we would most likely not have attempted it, if it were not for coding agents.

We pick JAX following the path of recent successful codes across scientific domains, for example JAXFLUIDS \cite{Bezgin2023} for Eulerian CFD, JAX-SPH \cite{toshev2024jax} for Lagrangian fluids, and TORAX \cite{citrin2024toraxfastdifferentiabletokamak} for fast Tokamak transport simulation.
%
...
%
By prioritizing a minimal and fully differentiable codebase, we bypass the architectural constraints of solvers like GKW, in the context of differentiable programming and learned surrogate models.
...
\label{sec:gyaradax}
\begin{itemize}
    \item stencils and other performance improvements
    \item details on our setup for vibecoding
\end{itemize}

\subsection{Speed}
\subsection{Vibecoding}
\label{sec:vibecoding}

\section{Validation}
\begin{itemize}
    \item validation against empirical gkw results
    \item test cases frok gkw repo
    \item verification of gradients against FD
\end{itemize}
...
\section{Experiments}
\begin{itemize}
    \item performance improvements against gyroswin and cgyro?
    \item maybe: train gyroswin with solver in the loop
\end{itemize}
\subsection{Performance}
\subsection{Solver-in-the-Loop + Gyroswin}

\section{Conclusion and Future Work}
\begin{itemize}
    \item conclusion ...
    \item full thing as future work
\end{itemize}
