...
class JAXOps(SolverOps):
    ...
    def _linear_rhs_terms(
        ...
        # term XI: curvature drift × F_M × bpar coupling (the bpar analog of VIII).
        # GKW vd_grad_phi_fm elem2 at linear_terms.f90:2664-2667. Equivalent to VIII
        # with gyro_phi replaced by bpar_chi_factor*bpar.
    ...
    def linear_rhs(
        ...
        if df.ndim == 5:
            ...
            shared = {
                ...
                "hyper": pre["hyper"],
                # dvp (parallel-velocity grid spacing) is species-independent;
                # needed by Term IV in _linear_rhs_terms (was params.dvp pre-merge)
                "dvp": pre["dvp"],
            }
