...
gyaradax's linear / NL solver.

Subclasses (`GyaradaxQLTransportModel`, `GyaradaxNLTransportModel`) implement
`_per_radius(params, geom) -> (qi, qe, pfe)` and inherit the vmap-over-rho_match
...
from torax._src.transport_model.quasilinear_transport_model import (
    QuasilinearInputs,
...
from gyaradax.geometry import build_topology, compute_continuous_geometry
...
def gkparams_for_radius(
    rho_idx, ql_inputs: QuasilinearInputs, core_profiles, geo, config,
) -> GKParams:
    ...
    return GKParams(
        rlt=rlt, rln=rln, q=q, shat=shat, eps=eps, beta=beta,
        ...
        dt=_GKPARAMS_DT, backend=backend,
    )
...
def gyaradax_geometry_at(
    ...
    return compute_continuous_geometry(
        ...
        ns=config.ns, nkx=config.nkx, nky=config.nky,
        nvpar=config.nvpar, nmu=config.nmu,
        vpar_max=_VPAR_MAX, nperiod=_NPERIOD, kxmax=_KXMAX,
        krhomax=_KRHOMAX, ikxspace=config.ikxspace,
        signB=_SIGNB, Rref=_RREF, geom_type=_GEOM_TYPE,
        topology=topology,
    )
...
def precompute_topology(config) -> Dict[str, Any]:
    ...
    return _get_topology_cached(config.nkx, config.nky, config.ikxspace, config.ns)


def initial_df(config) -> jnp.ndarray:
    """Initial df: cosine in s, every non-zonal ky seeded. Bypasses init_f's .item()."""
    nv, nmu, ns, nkx, nky = (
        config.nvpar, config.nmu, config.ns, config.nkx, config.nky,
    )
    ...
    def call_implementation(
        ...
        def per_radius(idx):
            ...
            geom = gyaradax_geometry_at(
                ...
                config=self, topology=self.topology,
            )
