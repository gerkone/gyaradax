...
def _load_generated_em_case(case, step):
    """Load generated EM validation data from GKW_EM_DATA_ROOT, skip if missing."""
    d = os.path.join(GKW_EM_DATA_ROOT, case, f"steps_{step:03d}")
    if not os.path.exists(d):
        pytest.skip(f"generated EM validation data not found at {d}")
    required = ["input.dat", "FDS", "FDS.dat", "geom.dat", "time.dat", "fluxes.dat"]
    missing = [name for name in required if not os.path.exists(os.path.join(d, name))]
    if missing:
        pytest.skip(f"generated EM validation data is incomplete at {d}: {missing}")
    return d
...
def _generated_case_ntimes(case_dir):
    """Return NTIME from a generated GKW input.dat."""
    input_data = _generated_case_input(case_dir)
    return int(input_data.get("control", {}).get("ntime", 1))


def _gkw_disables_linear_window_normalization(case_dir):
    """Return whether GKW input disables both linear normalization switches."""
    control = _generated_case_input(case_dir).get("control", {})
    return bool(
...
def _generated_case_state(case_dir):
    ...
    dg_gkw, info = load_gkw_dump(os.path.join(case_dir, "FDS"), shape, n_species=nsp)
    return geometry, params, pre, nsp, dg_gkw, info
...
class TestGeneratedEMValidationData:
    """Optional tests for generated short-step EM GKW validation data.

    These tests consume data generated by
    ``scripts/generate_em_gkw_validation_data.py`` under a layout such as
    ``$GKW_EM_DATA_ROOT/{apar_only,apar_bpar,bpar_only}/steps_001``.
    The same-state flux checks are strict diagnostic parity tests from the GKW
    FDS.  The FDS evolution checks use the same normalization handling as the
    existing strong FDS parity tests: when GKW disables ``normalized`` and
    ``normalize_per_toroidal_mode``, gyaradax's linear-window normalization is
    disabled in the test path too.
    """

    @pytest.mark.slow
    @pytest.mark.parametrize("step", [1, 5, 10, 20])
    @pytest.mark.parametrize("case", ["apar_only", "apar_bpar", "bpar_only"])
    def test_generated_fds_evolution_parity(self, case, step):
        """Generated EM FDS evolution matches GKW with GKW normalization flags honored."""
        case_dir = _load_generated_em_case(case, step)
        geometry, params, pre, nsp, dg_gkw, _ = _generated_case_state(case_dir)
        n_steps = _generated_case_ntimes(case_dir)
        assert n_steps == step

        if _gkw_disables_linear_window_normalization(case_dir):
            # Generated GKW inputs set normalized=.false. and
            # normalize_per_toroidal_mode=.false.; make gyaradax match by
            # preventing the solver's linear-window normalization from firing.
            # This mirrors the existing strong FDS parity handling.
            params = replace(params, naverage=10**9)

        dg = init_f(geometry, finit=params.finit, amp_init_real=params.amp_init, n_species=nsp)
        state = default_state(nky=len(geometry["krho"]))
        dg, _, _, _ = gk_run(dg, geometry, params, state, n_steps=n_steps, pre=pre)

        rel_l2 = float(jnp.linalg.norm((dg - dg_gkw).ravel()) / jnp.linalg.norm(dg_gkw.ravel()))
        assert rel_l2 < 1e-5, f"generated {case} steps={step} rel_l2={rel_l2:.6e} >= 1e-5"

    @pytest.mark.slow
    @pytest.mark.parametrize("step", [1, 5, 10, 20])
    @pytest.mark.parametrize("case", ["apar_only", "apar_bpar", "bpar_only"])
    def test_generated_fds_same_state_em_bpar_fluxes_match_gkw(self, case, step):
        """EM/Bpar flux diagnostics from generated GKW FDS match final GKW rows."""
        case_dir = _load_generated_em_case(case, step)
        geometry, params, pre, _, dg_gkw, info = _generated_case_state(case_dir)
        _, apar, bpar = _compute_fields(dg_gkw, geometry, params, pre)
        df = g_to_f(dg_gkw, apar, params, pre)

        time_rows = np.atleast_2d(np.loadtxt(os.path.join(case_dir, "time.dat")))
        ref_idx = int(np.argmin(np.abs(time_rows[:, 0] - float(info["time"]))))
        assert np.isclose(time_rows[ref_idx, 0], float(info["time"]), rtol=1e-10, atol=1e-10)

        comparisons = []
        if apar is not None and os.path.exists(os.path.join(case_dir, "fluxes_em.dat")):
            pred_em = np.asarray(
                calculate_em_fluxes(geometry, df, apar, params=params, bpar=None, pre=pre)
            ).reshape(-1)
            ref_em = np.atleast_2d(np.loadtxt(os.path.join(case_dir, "fluxes_em.dat")))[ref_idx]
            comparisons.append(("fluxes_em.dat", pred_em, ref_em))
        if bpar is not None and os.path.exists(os.path.join(case_dir, "fluxes_bpar.dat")):
            pred_bpar = np.asarray(
                calculate_em_fluxes(geometry, df, None, params=params, bpar=bpar, pre=pre)
            ).reshape(-1)
            ref_bpar = np.atleast_2d(np.loadtxt(os.path.join(case_dir, "fluxes_bpar.dat")))[ref_idx]
            comparisons.append(("fluxes_bpar.dat", pred_bpar, ref_bpar))

        assert comparisons, f"no EM/Bpar flux files available for {case_dir}"
        for name, pred, ref in comparisons:
            assert pred.shape == ref.shape == (6,)
            np.testing.assert_allclose(pred, ref, rtol=5e-5, atol=1e-10, err_msg=name)
