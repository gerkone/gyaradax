...
class RunningMeanStd:
    ...
    def combine(self, other: "RunningMeanStd") -> None:
        ...
        """
        assert (
            self.mean is not None and other.mean is not None
        ), "Cannot combine uninitialized RunningMeanStd objects"
...
def jax_spectral_shift(f, shift, axis):
    """
    Apply a spectral shift to a JAX array along a given axis.
    f: input array (complex)
    shift: amount to shift (can be non-integer)
    axis: axis along which to shift
    """
    n = f.shape[axis]
    # For a shift in index space, we multiply by exp(-2pi*i * k * shift / n)
    # where k is the frequency index [-n/2, n/2)
    k = jnp.fft.fftfreq(n) * n
    # Reshape k to match f's dimensions for broadcasting
    shape = [1] * f.ndim
    shape[axis] = n
    k = k.reshape(shape)

    # Apply phase shift in Fourier space
    # First, transform to real space along that axis
    # Wait, if f is already in spectral space (kx), we just need to multiply by phase
    # if shift is in units of kx indices.
    # Actually if f is f(kx), then a shift in x-space is multiplying by exp(i*kx*dx)
    # But Twist-and-Shift is a shift in KX-space itself.
    # To shift in kx-space, we transform to x-space, multiply by exp(i*x*dkx), then transform back.

    f_x = jnp.fft.ifft(jnp.fft.ifftshift(f, axes=axis), axis=axis)
    # x grid from -pi to pi? or 0 to 2pi?
    # standard fft uses 0 to 2pi effectively.
    x = jnp.linspace(0, 2 * jnp.pi, n, endpoint=False).reshape(shape)
    f_x_shifted = f_x * jnp.exp(1j * x * shift)

    return jnp.fft.fftshift(jnp.fft.fft(f_x_shifted, axis=axis), axes=axis)
