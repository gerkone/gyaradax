...
def run_benchmark():
    ...
    if args.time_average_flux:
        ...
        data_dir = (
            params.run.data_dir if hasattr(params, "run") else geometry.get("data_dir")
        )  # Setup simulation might not put it in params
        ...
        if not os.path.exists(args.reference):
            ...
            if error < 1e-12:
                ...
                print(
                    "[PASS] Numerical results match reference (single precision range)."
                )
            else:
